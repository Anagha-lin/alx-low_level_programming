...
if (file_from == -1)
{
	dprintf(STDERR_FILENO, "Error: if can't be read %s\n", argv[1]);
	exit(98);
}
...
if (file_to == -1)
{
	dprintf(STDERR_FILENO, "Error: if Can't write %s\n", argv[2]);
...
while (num1 == 1024)
{
	...
	if (num1 == -1)
	{
		dprintf(STDERR_FILENO, "Error: if Can't read %s\n", argv[1]);
		exit(98);
	}
...
if (close(file_from) == -1)
	dprintf(STDERR_FILENO, "Error: if Can't close fd %d\n", file_from), exit(100);
